...
Algorithm – shortest-first guillotine sequencing:
  ...
  4. Choose the cut whose length is shortest (fewest passes of the saw).
     If several cuts have the same length, choose the one that detaches
     the smallest sub-region from the sheet edge.
  5. That cut splits the region into two sub-regions. Recurse into the
     smaller cut-off piece first, then continue on the larger remainder.
...
def compute_cuts(layout: SheetLayout, kerf: float = 0.0) -> list[CutLine]:
    ...
    def _ordered_children(first: _Region, second: _Region) -> tuple[_Region, _Region]:
        children = sorted(
            [first, second],
            key=lambda child: (child.area(), child.y, child.x),
        )
